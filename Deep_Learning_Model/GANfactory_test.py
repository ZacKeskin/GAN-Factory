...
class GAN():
    ...
    # TEST the model
    def test(self,height,width,channel):
        current_dir = os.getcwd()
        ...
        result_path=[]
        for i in range (1,5):
            result_path.append(os.path.join(current_dir, 'Test/result/result'+ str(i) +'.jpg'))
        
        


        res_input, res_target=test_import_data('Test/input.jpg',height,width,channel,1), test_import_data('Test/target.jpg',height,width,channel,1)
        ...
        with tf.Session() as sess:
            ...
            #RESTORE
            saver = tf.train.import_meta_graph('checkpoints/test.meta')
            restore_path=tf.train.latest_checkpoint('checkpoints')
